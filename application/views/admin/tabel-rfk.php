				...
				<div class="card">
					<div class="card-body">
						...
						<div class="mb-5">
							<form action="<?= site_url('admin/detil_rfk') ?>" method="post">
								<div class="form-group row mb-2">
									...
									<div class="col-md-4">
										<select name="bulan" id="bulan-rfk" class="form-control" required>
						...
						<div class="mb-2" style="text-align: right">
							<button class="btn btn-sm btn-success waves-effect"><i class="mdi mdi-file-excel"></i> Excel</button>
						</div>
									...
									foreach ($kegiatan as $k) : ?>
										<?php if ($k->program_id == $p->id) : ?>
											<tr>
												<td class="text-center"
													style="font-weight: bold;"><?= get_abjad($keg++) ?>.
												</td>
										...
										foreach ($subkegiatan as $sk) : ?>
											<?php if ($sk->idk == $k->id && $sk->idp == $p->id) : ?>
												<tr>
													<td class="text-center"
														style="font-weight: bold;"><?= strtolower(get_abjad($subkeg++)) ?>
														.
													</td>
													<td style="font-weight: bold;"
														colspan="2"><?= $sk->nama_subkegiatan ?></td>
													<td style="font-weight: bold;" nowrap><?= rupiah($sk->pagu) ?></td>
													<td style="font-weight: bold;"
														nowrap><?= rupiah($sk->kontrak) ?></td>
													<td style="font-weight: bold;"
														nowrap><?= rupiah($sk->pagu - $sk->kontrak) ?></td>
												</tr>
											<?php endif; ?>
												...
												<?php foreach ($keuangan as $keu) : ?>
													<?php if (($i->subkeg == $sk->id) && ($i->idk == $k->id) && ($i->idp == $p->id) && ($keu->id_kontrak == $i->id_kontrak)) : ?>
														<tr>
															...
															<td class="text-center"
																nowrap><?= date('d-m-Y', strtotime($i->mulai)) ?></td>
															<td class="text-center"
																nowrap><?= date('d-m-Y', strtotime($i->selesai)) ?></td>
															<td><?= $i->penyedia ?></td>
															<td><?= $i->no_kontrak ?></td>
														</tr>
													<?php endif; ?>
